...
mod.sit<-gls(rich~logCa+slope+northness+yr.f+dist.m,cor=corAR1(form=~yr|name),meth="REML")
mod.sit<-gls(scale(rich)~scale(logCa)+scale(slope)+scale(northness)+yr.f+scale(dist.m),cor=corAR1(form=~yr|name),meth="REML")

plot(ACF(mod.sit,maxLag=7,resType='r'),alpha=0.01)
plot(ACF(mod.sit,maxLag=7,resType='n'),alpha=0.01)

round(summary(mod.sit)$t,3)
               Value Std.Error t-value p-value
(Intercept)  175.145    20.857   8.398   0.000
logCa        -30.882     6.129  -5.038   0.000
slope         -0.807     0.588  -1.372   0.172
northness      2.863     1.430   2.002   0.047
                     Value Std.Error t-value p-value
(Intercept)         -0.612     0.163  -3.749   0.000
scale(logCa)        -0.417     0.083  -5.038   0.000
scale(slope)        -0.122     0.089  -1.372   0.172
scale(northness)     0.173     0.087   2.002   0.047

anova(mod.sit,type='m')
Denom. DF: 203 
            numDF  F-value p-value
(Intercept)     1 70.51889  <.0001
logCa           1 25.38528  <.0001
slope           1  1.88220  0.1716
northness       1  4.00984  0.0466
yr.f           10 13.17191  <.0001
dist.m          3  9.94677  <.0001
...
mod.clim1<-update(mod.clim,cor=corAR1(form=~yr|name),meth="ML")
anova(mod.clim,mod.clim1)
##AR1 mod sig better
plot(ACF(mod.clim1,resType='r'),alpha=0.05)
plot(ACF(mod.clim1,resType='n'),alpha=0.05)
##the AR1 model does a good job#
...
mod.dist1<-update(mod.dist,cor=corAR1(form=~yr|name),meth="ML")
anova(mod.dist,mod.dist1)
##temp autocorr needs to be corrected 
##visual inspection
plot(ACF(mod.dist1,resType='r'),alpha=0.05)
plot(ACF(mod.dist1,resType='n'),alpha=0.05)

mod.dist<-update(mod.dist1,meth="REML")
mod.dist<-gls(scale(rich)~scale(YrsOB)+scale(YrsSLB)+scale(BP5Yrs)+name+yr.f,cor=corAR1(form=~yr|name))

round(summary(mod.dist)$t,3)
                    Value Std.Error t-value p-value
(Intercept)        84.288     4.410  19.114   0.000
YrsOB               1.374     0.397   3.464   0.001
YrsSLB             -0.864     0.363  -2.381   0.018
BP5Yrs             -1.406     0.838  -1.678   0.095

                   Value Std.Error t-value p-value
(Intercept)        0.639     0.313   2.041   0.043
scale(YrsOB)       0.455     0.131   3.464   0.001
scale(YrsSLB)     -0.144     0.061  -2.381   0.018
scale(BP5Yrs)     -0.152     0.091  -1.678   0.095
...
YrsOB           1  11.9974  0.0007
YrsSLB          1   5.6695  0.0183
BP5Yrs          1   2.8157  0.0950
...
##for table 2
mod.dist<-gls(rich~dist.m+name+yr.f,cor=corAR1(form=~yr|name))
anova(mod.dist,type='m')
Denom. DF: 187 
            numDF  F-value p-value
(Intercept)     1 365.3284  <.0001
