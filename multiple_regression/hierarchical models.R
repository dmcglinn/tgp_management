...
gls.full<-gls(rich~-1+name+yr.f+YrsOB+bison+BP5Yrs+YrsSLB,meth="ML")
...
gls.full      1 35 1488.104 1606.881 -709.0521                        
gls.full2     2 36 1485.074 1607.244 -706.5369 1 vs 2 5.030447  0.0249
##to get sweet partial plots
coef(gls.full2)
mod.lm<-lm(rich~name+yr.f+YrsOB+BP5Yrs+YrsSLB+bison)
summary(mod.lm)
YrsOB               1.4385     0.3218   4.470 1.36e-05 ***
BP5Yrs             -2.0401     0.7541  -2.705 0.007457 ** 
YrsSLB             -1.0160     0.3385  -3.001 0.003059 ** 
bison              -0.2570     1.9427  -0.132 0.894893  
library(hier.part)
hier.part(residuals(lm(rich~name+yr.f)),data.frame(YrsOB=YrsOB,bison=bison,BP5Yrs=BP5Yrs,YrsSLB=YrsSLB),gof='Rsqu')
hier.part(rich,data.frame(YrsOB=YrsOB,bison=bison,BP5Yrs=BP5Yrs,YrsSLB=YrsSLB),gof='Rsqu')
hier.part(rich,data.frame(name=name,yr.f=yr.f,logCa=logCa,rain=rain.tot,temp=temp.avg,YrsOB=YrsOB,bison=bison,BP5Yrs=BP5Yrs,YrsSLB=YrsSLB),gof='Rsqu')
hier.part(rich,data.frame(name=name,logCa=logCa,rain=rain.tot,temp=temp.avg,YrsOB=YrsOB,bison=bison,BP5Yrs=BP5Yrs,YrsSLB=YrsSLB),gof='Rsqu')
hier.part(rich,data.frame(logCa=logCa,rain=rain.tot,temp=temp.avg,YrsOB=YrsOB,bison=bison,BP5Yrs=BP5Yrs,YrsSLB=YrsSLB),gof='Rsqu')


mod.lm<-gls(rich~-1+name+yr.f+YrsOB+BP5Yrs+YrsSLB+bison)
anova(mod.lm,type='m')
Denom. DF: 186 
...
par(mfrow=c(2,3))
termplot(mod.lm,partial=TRUE,se=T,smooth=panel.smooth,span.smth=1/3,lwd.term=1.5,lwd.se=2,ylim=c(-28,28))
...
gls.site<-gls(resids~YrsOB+bison+YrsSLB+BP5Yrs+logCa.avg+northness,cor=corAR1(form=~yr|name),meth="ML")
...
gls.time<-gls(resids~YrsOB+bison+YrsSLB+BP5Yrs+rain.mat+prain.mat+temp.mat,cor=corAR1(form=~yr|name),meth="ML")
...
gls.name<-gls(rich~name,cor=corAR1(form=~yr|name),meth="ML")
resids<-residuals(gls.name,type='n')
gls.time<-gls(resids~YrsOB+bison+YrsSLB+BP5Yrs+rain.tot*temp.avg,cor=corAR1(form=~yr|name),meth="ML")
round(summary(gls.time)$tTable,3)
plot(ACF(gls.time,maxLag=5,resType='r'),alpha=0.05)
plot(ACF(gls.time,maxLag=5,resType='n'),alpha=0.05)
plot(gls.time)
...
cyr2<-cyr^2
cyr3<-cyr^3
mod.lme<-lme(rich~cyr+logCa.avg+YrsOB+bison+YrsSLB+BP5Yrs+rain.tot*temp.avg,random=~1|name,cor=corAR1(form=~yr|name),meth="ML")
mod2.lme<-lme(rich~cyr+cyr2+logCa.avg+YrsOB+bison+YrsSLB+BP5Yrs+rain.tot*temp.avg,random=~1|name,cor=corAR1(form=~yr|name),meth="ML")
...
mod.lme<-lme(rich~syr+syr2+slogca+syob+bison+syslb+sbp5+srain*stemp,random=~1|name,cor=corAR1(form=~yr|name),meth="ML")
summary(mod.lme)
